...
- 严格验证验证码图片的 Content-Type 后规范化为基础 MIME，支持合法参数形式，再转换为 `640 x 640` CHW `Float32` 输入。
