...
const buildScriptPath = path.join(repositoryRoot, 'scripts', 'ort-runtime', 'build-minimal-ort-runtime.sh')
...
test('every ONNX conversion requirement has an exact version and hashes', async () => {
  const [requirements, input] = await Promise.all([readFile(requirementsPath, 'utf8'), readFile(inputPath, 'utf8')])
