...
export function normalizeBrowserVersionForComparison(version) {
  if (typeof version !== 'string' || !/^\d+(?:\.\d+)+$/u.test(version)) {
    throw new Error(`Unable to normalize browser version: ${version || 'unknown'}`)
  }
  const parts = version.split('.')
  while (parts.length > 2 && parts.at(-1) === '0') {
    parts.pop()
  }
  return parts.join('.')
}

export function assertBrowserVersionsEquivalent(expected, actual) {
  const normalizedExpected = normalizeBrowserVersionForComparison(expected)
  const normalizedActual = normalizeBrowserVersionForComparison(actual)
  if (normalizedExpected !== normalizedActual) {
    throw new Error(`Browser versions differ: expected ${expected}, received ${actual}`)
  }
