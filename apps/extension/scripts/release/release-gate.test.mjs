...
test('verify-monorepo pins Firefox load tooling and binds Android evidence to the release revision', async () => {
  ...
  assert.match(
    extensionJob,
    /- name: Install pinned geckodriver\n\s+id: geckodriver\n\s+run: node apps\/extension\/scripts\/browser\/install-geckodriver\.mjs "\$RUNNER_TEMP\/geckodriver"/u,
  )
  assert.match(
    extensionJob,
...
test(
  ...
  async (context) => {
    ...
    const run = () =>
      spawnSync(process.execPath, ['scripts/release/release-gate.mjs', 'preflight', '--output-root', outputRoot], {
