...
registerBroker((request) => {
  if (request.type === 'verify-key') {
    return Promise.resolve(errorResponse(request.requestId, '测试 Host 不接受模型 Key'))
  }
  ...
  return Promise.resolve(successResponse(request.requestId))
})
