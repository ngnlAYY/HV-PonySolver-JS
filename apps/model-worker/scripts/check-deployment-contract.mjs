...
function assertOptionsContract(response, origin) {
  const context = `OPTIONS origin=${origin}`
...
function assertHeadContract(response, origin, invalidKeyMode) {
  const context = `HEAD origin=${origin}`
...
async function checkContractRound({ fetchImpl, modelUrl, invalidKeyMode, origins, requestTimeoutMs }) {
  for (const origin of origins) {
    ...
    assertOptionsContract(optionsResponse, origin)
    ...
    assertHeadContract(headResponse, origin, invalidKeyMode)
  }
}
...
async function runCli({
  ...
  try {
    ...
    stdout.write(
      `Model Worker deployment contract verified: attempt=${result.attempt}/${result.attempts} mode=${result.invalidKeyMode} origins=${result.origins.length}\n`,
    )
    return 0
  } catch (error) {
